		...
		<c:forEach var="list" items="${list}">
			<div class="newsbox">
				...
				<div class="newscontent"> 
					<a href="<%=request.getContextPath()%>/health/detailtopics?id=${list.id}"><h2>${list.title}</h2></a>
					<div class="newscontentin">
						${list.content}
					</div>
					<a href="<%=request.getContextPath()%>/health/detailtopics?id=${list.id}"><button>Read more</button></a>
				</div>
			</div>
		</c:forEach>
	</div>
