...
public class TopicController {
  ...
  @RequestMapping(value="/health/modifytopics", method=RequestMethod.GET)
  public String modifyGet(Model model,Integer id) {
    TopicVo topic = topicservice.detailTopics(id);
    if(topic==null)  //(직접 URI를 입력하는 등의)잘못된 경로로 접근했을때는 null값이 반환되므로 이때는 게시글목록으로 보내줌
      return "redirect:/health/detailtopics";
    model.addAttribute("topic",topic);
    return "health/modifytopics";
  }
  
  @RequestMapping(value="/health/modifytopics", method=RequestMethod.POST)
